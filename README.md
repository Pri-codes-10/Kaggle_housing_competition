...
# Rank 34 out of 4,000 teams (Top 1%)

<img width="1576" height="901" alt="image" src="https://github.com/user-attachments/assets/694a8a43-b435-44ff-97fe-b0ae3c681a88" />


# House Prices: Advanced Regression Techniques (Kaggle Solution)

A robust, multi-model ensemble pipeline built for the Kaggle **"House Prices: Advanced Regression Techniques"** competition. By integrating a state-of-the-art tabular foundation model with advanced gradient boosters and regularized linear models, this pipeline achieved an exceptional leaderboard standing, securing **Rank 34 out of 4,000 teams (Top 1%)** using out-of-fold cross-validation and weighted blending.

---

## 🚀 Pipeline Architecture

The solution uses a 5-fold cross-validation strategy combined with a weighted blend of six distinct models to minimize variance and generalization error:

1. **TabPFN Regressor (Prior Labs)**: State-of-the-art tabular foundation model utilizing in-context learning via transformer architectures to capture complex non-linear patterns.
2. **XGBoost Regressor**: Optimized gradient boosting with hyperparameter fine-tuning (`max_depth=4`, `learning_rate=0.012`).
3. **LightGBM Regressor**: Fast, leaf-wise tree growth focusing on robust split patterns.
4. **LassoCV**: Linear model with L1 regularization via cross-validation and robust scaling.
5. **RidgeCV**: Linear model with L2 regularization to stabilize coefficient weights.
6. **ElasticNetCV**: Combined L1 and L2 regularization for handling multicollinearity among features.
...
* **Feature Engineering**: Generated high-value interaction and composite features:
  ...
  * Quality interaction terms (e.g., `OverallQual * TotalSF`)
* **Skew Correction**: Automatically detected numeric features with high positive skew (`> 0.75`) and applied `log1p` transformation.
* **Encoding & Scaling**: Applied robust scaling and one-hot encoding aligned safely across train and test sets.
...
```bash
pip install numpy pandas scikit-learn xgboost lightgbm scipy tabpfn-client
